...
structured results through the CLI, Python API, and a local MCP server.

The core rule of the project is simple: review logic stays in
`src/content_review_engine/`, while CLI, API, and MCP remain adapters over
the same workflow.

## Highlights

- Deterministic Markdown review with structured findings and source locations
- YAML review profiles with built-in templates and standalone validation
...
The repository is no longer in its initialization phase. The current surface
includes:

- core Markdown parsing, profile loading, rule execution, and report rendering
...
The deterministic review layer is still the most stable automation contract
today.
...
- Markdown heading and paragraph structure issues
- obvious Markdown link and image placeholder issues

Profiles can also control:

- enabled rules
- severity thresholds
- allowlists
- title and paragraph length limits
- custom regex rules

Built-in profiles live under [profiles/examples](profiles/examples).

## Output Model

The project intentionally keeps deterministic, LLM, and combined artifacts as
three separate output families:
...
- deterministic output remains the main automation contract
...
The repository includes a local MCP server wrapper over the same Python API
facade.
...
The LLM layer is an adapter around the deterministic pipeline, not a
replacement for it.
...
profiles/examples/           built-in profile templates
...
- The project currently reviews Markdown only.
- Deterministic rules are explicit, configurable, and profile-driven.
- LLM review is advisory by default unless you explicitly enable its own gate.
- The project does not guarantee legal, medical, advertising, regulatory, or
  platform compliance.
- MCP support is currently documented primarily for local `stdio` usage.
