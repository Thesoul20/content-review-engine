...
def test_cli_docs_define_output_boundaries_and_behavior_matrix() -> None:
    ...
    assert "## LLM CLI Behavior Matrix" in content
    ...
    assert "`--llm-fail-on` without `--enable-llm` is a CLI usage error" in content
    assert "provider selection/config controls LLM execution only" in content
...
def test_architecture_and_data_model_docs_keep_combined_boundaries_aligned() -> None:
    ...
    assert "providers do not own deterministic `--fail-on`" in architecture
    ...
    assert "provider or provider-config metadata in raw LLM sidecars remains execution" in data_models
...
def test_usage_and_ci_docs_explain_deterministic_only_quality_gate() -> None:
    ...
    assert "## Provider Responsibility Boundary" in usage
    ...
    assert "provider selection or provider failure does not change which gate owns exit" in ci
